...
void ble_event(BLE_PROXIMITY_EVENT eventArgs) {
  ...
  if (beacons.length() == 14) {
    ...
    if (beacon1.substring(0, 2) == "03"  && beacon1.substring(4, 6).toInt() < 75) {
      patientNotInBed = 0;
      Serial.println("");
      Serial.print("Two beacons, patient in the bed "); Serial.println(patientNotInBed);
    }
    if (beacon1.substring(0, 2) == "03" && beacon1.substring(4, 6).toInt() > 75) {
      patientNotInBed++;
      Serial.println("");
      Serial.print("Two beacons, patient NOT in the bed "); Serial.println(patientNotInBed);
    }
